...
local function test_init_only()
    ...
    local model = nn.InitUpdateRecurrent(
        init, update, math.huge --[[reinitialize_rate]], 1 --[[rho]])
...
local function test_reinit()
    ...
    local update = nn.MulConstant(2)
    local model = nn.InitUpdateRecurrent(
        init, update, 3 --[[reinitialize_rate]], 1 --[[rho]])
...
local function test_check_sharedClones_table_sanity()
    ...
    local update = nn.MulConstant(2)
    local model = nn.InitUpdateRecurrent(
        init, update, math.huge--[[reinitialize_rate]], 2 --[[rho]])

    model:forward(torch.rand(5, 5))
    model:forward(torch.rand(5, 5))
    model:forward(torch.rand(5, 5))
    model:forward(torch.rand(5, 5))
    assert(__.size(model.sharedClones) == 4)
    assert(model.sharedClones[1] == model.initialModule)
    assert(model.sharedClones[2] == model.recurrentModule)
    assert(model.sharedClones[3].constant_scalar == 2)
    assert(model.sharedClones[4].constant_scalar == 2)
    ...
    assert(model.sharedClones[5].constant_scalar == 2)
    assert(model.sharedClones[6].constant_scalar == 2)
    assert(model.sharedClones[7].constant_scalar == 2)
    assert(__.size(model.sharedClones) == 4)

    model:forget()
    assert(model.sharedClones[1] == model.initialModule)
    assert(model.sharedClones[2].constant_scalar == 2)
    assert(model.sharedClones[3].constant_scalar == 2)
    assert(model.sharedClones[4].constant_scalar == 2)
...
local function test_check_sharedClones_table_sanity_reinit()
    local init = nn.MulConstant(1)
    local update = nn.MulConstant(2)
    local model = nn.InitUpdateRecurrent(
        init, update, 3 --[[reinitialize_rate]], 2 --[[rho]])
    ...
    assert(model.sharedClones[2].constant_scalar == 2)
    assert(model.sharedClones[3].constant_scalar == 2)
    assert(model.sharedClones[4].constant_scalar == 1)
    ...
    assert(model.sharedClones[5].constant_scalar == 2)
    assert(model.sharedClones[6].constant_scalar == 2)
    assert(model.sharedClones[7].constant_scalar == 1)
    assert(__.size(model.sharedClones) == 3)
...
local function test_load_save()
    ...
    local update = nn.MulConstant(2)
    local model = nn.InitUpdateRecurrent(
        init, update, nil --[[reinitialize_rate]], 1 --[[rho]])
...
test_util.run_test(test_reinit, 'Simple reinit')
test_util.run_test(test_updates, 'Updates')
test_util.run_test(test_check_sharedClones_table_sanity, 'sharedClones sanity')
test_util.run_test(test_check_sharedClones_table_sanity_reinit,
                   'sharedClones sanity with reinit')
